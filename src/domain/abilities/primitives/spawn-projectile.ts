 ...
 * A homing entry gives it the cast's target, so it turns onto that unit every tick and lands
 * on it alone; a linear one holds the bearing and sweeps whatever hostile crosses it.
 *
...
export const spawnProjectile: Primitive<SpawnProjectileEffectDef> = (
  ...
  const id: EntityId | null = acquireProjectile(
    world,
    cast.anchor.x,
    cast.anchor.y,
    cast.facing,
