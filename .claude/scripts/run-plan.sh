#!/usr/bin/env bash
# Run the implementation plan until phase 5 closes, one fresh Claude Code session per ticket.
#
#   .claude/scripts/run-plan.sh
#
...
# then writes a sentinel file. The runner checks the commit, closes the session and starts the
# next ticket. Rerunning resumes from wherever STATUS.md points.
#
# Validation by a person waits until phase 5 is done; the sessions record it and move on.
...
if [[ -z "${TMUX:-}" ]]; then
  ...
  tmux new-session -d -s "$TMUX_SESSION" -n runner -c "$REPO_ROOT" \
    "bash '$0'; echo; echo 'Runner exited. Press enter to close.'; read"
  echo "Plan runner started in tmux session '$TMUX_SESSION'."
...
phase_five_closed() { status_row "Last closed phase" | grep -qE '\| \[5 '; }
...
- Validation by a person waits until phase 5 is done. That is the maintainer's standing instruction of 2026-09-24. It covers walks by hand, approvals by the product owner or maintainer, looks by eye, benchmarks or browser checks only a person can run, and the reference-laptop numbers. None of them blocks a ticket, a sprint or a phase gate before then, and none of them is a reason to notify me. For each one, write an open checkbox under "Waiting on a person" in STATUS.md with the steps, where the result goes and what it confirms, and say it is deferred until phase 5 is done by that instruction. A phase gate row that needs a person also goes to backlog/deferred.md. Then close the ticket, sprint or phase on the rows an agent can verify, with a note that the person rows are deferred.
...
- Do not send notifications for routine progress. If this ticket closes phase 5, send a PushNotification saying the plan is finished and how many rows wait on me under "Waiting on a person".
- Commit is authorised. When the ticket is done, \`pnpm check\` has passed, its Status row is \`done\` and STATUS.md is updated, stage everything this ticket changed and make exactly one commit in the style of this repository's git log: a "Close $ticket: <what changed>" subject, a blank line, prose paragraphs, and the Co-Authored-By line. Never push.$( [[ -n "$keep_out" ]] && printf '\n- Never stage these paths; they are the maintainer'"'"'s:\n%s' "$keep_out" )
...
log "plan runner on $BRANCH, next: $(pick_ticket)"

n=0
until phase_five_closed; do
  ticket="$(pick_ticket)"
  if [[ -z "$ticket" ]]; then
    alert "stopped: STATUS.md names no ticket in progress or next, and phase 5 is not closed"
    exit 1
  fi
...
log "phase 5 is closed after $n tickets this run; $(open_person_rows) rows wait on a person in STATUS.md"
