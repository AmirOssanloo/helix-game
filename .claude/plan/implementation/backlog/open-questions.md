...
| Q5 | What is the tuning key for a definition field? | `def:<kind>:<id>:<field path>[:<index>]`, numeric fields only, copied into run scope at world creation and read by every system through the tuning state | P4-S17-T01 | Proposed |
...
| Q18 | Definition field names mix cases: the content standard's own example writes `cooldown_seconds` beside `atlasFrame`, and the coding standard's snake_case row names ids and keys but not fields. Which case do definition fields take? | snake_case for every definition field, because a field path is part of a tuning key (Q5) and reads the same in a definition, the tuning table, and a panel label. The runbook examples follow the ticket that raised this (`cast_point_seconds`, `cooldown_seconds`, `mana_cost`, `attack_point_seconds`, `base_attack_seconds`) and leave the other fields as written; the first definition type settles the rest and the content standard's example is fixed in the same change | P1-S04-T03 | Proposed |
