...
Open. P5-S22-T01 walked the gate on 2026-09-25. The phase closes when P5-S22-T02's docs sync lands, since that ticket's checklist is the docs row's evidence. The rows that need a person are deferred until phase 5 is done by the maintainer's standing instruction of 2026-09-24. Sized versus actual is to date, and P5-S22-T04 completes it.

| Row | Result | Recorded by |
| --- | --- | --- |
| Gate rows | Walked 2026-09-25; the evidence per row is in the [sprint 22 gate walk](./sprint-22-phase-gate-and-handover.md#phase-5-gate-walk). Five of eight hold outright: the pipeline diff, the disable matrix, the tiers, the roster, and the bar's headless half. The boss encounter holds headless. The docs row waits on P5-S22-T02. The reference-laptop row, the boss encounter per browser, and the bar per browser wait on a person, as open boxes in STATUS.md and rows of [Deferred](../backlog/deferred.md). One gate bug: `pnpm check:ci` timed out two corridor replay cases under coverage, fixed as P5-S22-T05 | the engineer running the plan |
...
| Sized versus actual | To date: sized 16.05 days, 16 planned and 0.05 unplanned (P5-S22-T05). Sprints 19 to 21 took 1.6, 1.1, and 1.4 against 4 each. Sprint 22 has taken 0.35 so far, T01 at 0.3 and T05 at 0.05, against 4.05. That is 4.45 actual against the 12.05 sized for the work done, a ratio of 0.37 | the engineer running the plan |
| Largest miss | To date, no ticket went over its size. The widest gaps were P5-S20-T02, the matrix as data and the validator, sized 2 and done in 0.5, and P5-S21-T02, the roster, sized 2 and done in 0.6. The one number that moved was the boss variant's single worst tick of 3.19 ms, which Q48 holds provisional against the cap rule | the engineer running the plan |
