...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. Grunts at the three tiers held 400, 1200, and 4000. The boss's outline is wider and reads; the elite's red outline barely shows on the red body, in Deferred for the art pass. The elite slammed, the boss charged when the hero walked away and cast its heal below half, and Hoarfrost stunned it on each hit as it stuns a grunt. Note the **No cooldowns** switch frees enemy clocks too, so enemy checks are walked with it off.
...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. The Enemies list holds fifteen. The nine, spawned in packs of three, each read with the catalogue's frame, size, and colour; the indigo hexer and the sky-blue lancer are the closest pair and read apart up close. Nothing differs from the catalogue.
...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. The hexer and the skirmisher back away while their clock runs, turn and fire on each clock, and fire on a hero that steps in 40 units without moving; the hero at 280 still catches a kiter at 250. The lancer charged from about 630, held at about 598 when the hero walked away, and charged again 12.3 s after its first. Behind a wall it stood pinned for its whole clock; Q47 is answered with the charger closing when the hero is nearer than its hold point, built as P5-S22-T07.
...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. Behaviour and tick in Chrome: the boss brute bashed, slammed, brought two imps, and charged; Units went from 199 to 201 and never past it. Chrome's tick over the last 300 ticks: 0.35 ms mean, 0.5 ms p99, 0.6 ms max, no event overwrites, no pool misses. The readouts per browser on the reference laptop wait for the hardware sitting.
