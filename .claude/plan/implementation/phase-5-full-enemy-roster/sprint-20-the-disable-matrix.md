...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. The playable outcome holds in the arena. D held open on Hoarfrost as the hexer's wind-up ended: the curse closed the cursor with mana at 255 and no flash, and A pressed under the silence opened a cursor whose click issued the attack-move. A net landing mid-walk stopped the hero, and it stood when the root ended. A bash landing inside the hero's Hoarfrost cast point cancelled it with the mana and the clock untouched. The disable matrix is approved as written; Q43 and Q44 answered as decided.
