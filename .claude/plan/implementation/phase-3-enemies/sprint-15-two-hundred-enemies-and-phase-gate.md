...
**Walked, 2026-09-25, by the delivery lead on the maintainer's delegation**, in Chrome on the Apple M1 laptop against `pnpm dev`, the simulation advanced by the panel's single step because the tab reported itself hidden; frame rate, render time, draw calls, and the bench need a visible window and wait for the hardware sitting in STATUS.md. The corridor crowd at four passes reads as a crowd: its closest pair stayed 26.6 apart, no two squares on one point. But ten grunt packs and ten runner packs carried the hero about 600 units out of the corridor in fifteen seconds with a click back every half second, and it died once when not healed. Q31 is answered: the hero will take a smaller share of push-out, the first ticket of the next bet, in Deferred.
