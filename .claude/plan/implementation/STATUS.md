...
| **Next ticket** | P2-S07-T02 — Definition types, schemas, the registry, and the content tier |
| **In progress** | none |
| **Last closed ticket** | P2-S07-T01 — The spell catalogue, 2026-09-21: `docs/product/specs/spell-catalogue.md`, the ten spells as data with three named effects and the shopping list for sprints 08 to 11 |
...
- [ ] **Sprint 07: the product owner approves the spell catalogue's shape.** Read `docs/product/specs/spell-catalogue.md`, sections 3, 4, and 7: the effect list of each spell, the three named effects, the primitive parameters, and the status-definition capabilities. Approve as written, or say which entry changes and why, in a note under P2-S07-T01 in the sprint 07 file. The numbers are starting values and need no approval; the tuning surface changes them later. The three calls most worth a look: cooldown and mana index by the lowest orb level in the recipe while every other table names its orb; a status definition carries no duration, the applier does; and Bolide, Zenith, and Wane are primitives only because a zone carries effect lists instead of a rule key. Unblocks: P2-S07-T02, the definition schemas, which is not started against a draft.
