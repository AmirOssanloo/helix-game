...
| **Next ticket** | P2-S08-T04 — Statuses on screen: icons and greyed keys |
| **In progress** | none |
| **Last closed ticket** | P2-S08-T03 — The zone entity and system, the spawn-zone primitive, 2026-09-22: a zone is an area with a clock, holding the two effect lists its entry gave it and running them through the same runner a commit uses; the system carries a travelling or caster-anchored zone and expires it exactly on its tick; the panel spawns a bare circle, the ground band draws every zone, and the spell-areas overlay outlines them as the simulation tests them |
