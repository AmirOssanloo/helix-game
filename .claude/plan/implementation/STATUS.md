...
| **Next ticket** | P2-S08-T03 — The zone entity and system, the spawn-zone primitive |
| **In progress** | none |
| **Last closed ticket** | P2-S08-T02 — Primitives: damage area, apply status, displace, 2026-09-22: three of the six primitives run, the three shape tests are pure functions in `domain/movement/`, an effect collects only units hostile to the caster and never a corpse or a lifted one, a push is a record the movement step carries into the collision pass, and a lift puts the order aside and gives it back on the drop |
