...
| **Active sprint** | [09 — Projectiles, summons, the dummy, and the auto-attack](./phase-2-spells-and-attack/sprint-09-projectiles-summons-and-the-auto-attack.md) |
| **Next ticket** | P2-S09-T04 — Hit feedback from events: flashes and floating numbers |
| **In progress** | none |
| **Last closed ticket** | P2-S09-T03 — The training dummy and the auto-attack, 2026-09-22: the training dummy stands in the arena from a registry-driven dropdown, and the hero walks into reach, faces, and fires a homing shot every base attack time; an attack is its own block of numbers on the hero definition and every archetype, carried out by `src/domain/attack/` over the same state machine, turn, and projectile a cast uses; an attack-move acquires inside its radius and takes its walk up again where it stands; and `summon_follow` attacks what is near before it follows |
| **Last closed sprint** | [08 — Statuses, disable flags, zones, and the primitives](./phase-2-spells-and-attack/sprint-08-statuses-combat-and-the-primitives.md), 2026-09-22: the status table, the system, and the eight disable flags run; damage-area, apply-status, and displace are built over them; a zone is an area with a clock running two effect lists; and the statuses now read on screen. Four tickets sized four days landed in 2.0 |
