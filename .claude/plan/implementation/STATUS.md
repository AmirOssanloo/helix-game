...
| **Next ticket** | P2-S08-T02 — Primitives: damage area, apply status, displace |
| **In progress** | none |
| **Last closed ticket** | P2-S08-T01 — Status table, status system, disable flags, the eight status definitions, 2026-09-22: `applyStatus` is the one door with `writeStatus` the stack rule behind it, run scope holds every status with its tables read for the tick, the status pass runs second and derives the eight flags and the status modifier rows afresh each tick, the panel applies any status by id, and `status_applied` and `status_expired` are in the event union |
