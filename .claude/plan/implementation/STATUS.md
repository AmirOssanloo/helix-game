...
| **Next ticket** | P2-S09-T03 — The training dummy and the auto-attack |
| **In progress** | none |
| **Last closed ticket** | P2-S09-T02 — Summons: the spawn-unit primitive, ownership, lifetime, 2026-09-22: `spawn_unit` dresses a summon from the definition it names, writes the entry's bonuses on it as modifier rows, and gives it an owner and a lifetime; a pass of its own runs each unit's behaviour once a tick, and `summon_follow` keeps the summon on its owner's heel; the death pass releases it on its lifetime and on the tick its owner dies, silently and granting nothing |
