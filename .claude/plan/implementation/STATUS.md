...
| **Active sprint** | [08 — Statuses, disable flags, zones, and the primitives](./phase-2-spells-and-attack/sprint-08-statuses-combat-and-the-primitives.md) |
| **Next ticket** | P2-S08-T01 — Status table, status system, disable flags, the eight status definitions |
| **In progress** | none |
| **Last closed ticket** | P2-S07-T04 — Damage types, mitigation, and death resolution, 2026-09-22: `applyDamage` is the one door and `mitigate` the rule behind it, the armour curve and the corpse delay are tunables, `unit_damaged` and `unit_died` are in the event union with the panel reading both, and the death system now takes every unit with a health pool, the hero respawning and every other unit releasing its slot after the delay |
| **Last closed sprint** | [07 — Spell catalogue, definition schemas, the registry, and the pipeline](./phase-2-spells-and-attack/sprint-07-spell-catalogue-and-the-ability-pipeline.md), 2026-09-22: the ten spells are specified, every definition kind has a schema the registry validates, the pipeline is the real one with the effect runner behind it, and damage, mitigation, and death are in place; four tickets sized four days landed in 2.5 |
