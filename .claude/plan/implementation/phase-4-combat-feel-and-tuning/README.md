...
| Presentation sync | Under 1 ms | Not measurable headless: in Node Phaser is the test stub, so a sync time would leave out its setters. The last browser figure is 0.77 ms mean with 201 bound, in Chrome on the Apple M1 laptop at the phase 3 close. At the phase 4 cap, waiting on a person | Waiting on a person; 0.23 ms on the phase 3 figure |
| Phaser render and world draw calls | Under 6 ms; under 5 draw calls | Waiting on a person, per browser | Waiting on a person |
| Frame rate | 60 fps stable in four browsers | Waiting on a person, per browser | Waiting on a person |
...
| Heap flat | Flat over five minutes | 25.62 MB at thirty seconds, 25.72 MB at five minutes, never above 25.77, after full collections (P4-S18-T02). In a browser, waiting on a person | 0.10 MB over five minutes |
...
| Render benchmark | ADR 0001 | Waiting on a person, `pnpm bench` in Chrome at the phase 4 close | Waiting on a person |
