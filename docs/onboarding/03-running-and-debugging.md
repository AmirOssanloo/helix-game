...
| Simulation   | Pause, Single-step, Catch-up cap, Seed, Save input log, Load input log, Reset map                                                                      | Freezes and steps the world; records and replays a session |
...
| Readouts     | Frame rate, Tick time (mean and max over the last second), Render time, Draw calls, Live units, Live projectiles, Live zones, Pool misses, Heap           | Live numbers from the instrumentation rings and the renderer counters |
...
This ticks 300 units with random orders on a 4000 by 4000 arena in Node, with no renderer, and asserts a mean tick under 4 ms. It is part of `pnpm test`, so it runs before every push; run it alone when you have touched movement, pathing, collision, or AI and want the number quickly.
