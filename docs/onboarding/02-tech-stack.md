...
It is the only runtime dependency besides Phaser, and the only one a production build does not carry: the pane ships where the panel does and nowhere else, and the build refuses a production bundle holding either. A binding reports a change when a person makes one and when the panel rewrites it from the world, which is why a control that follows the world compares before it acts. [Developer tools and instrumentation](../architecture/devtools-and-instrumentation.md) holds that rule.
