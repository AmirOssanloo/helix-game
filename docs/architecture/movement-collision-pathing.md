...
- **Unit against unit:** two discs closer than the sum of their collision radii are pushed apart along the line between their centres until they touch. A unit in the air is still a disc but one nothing moves: the unit on the ground takes the whole overlap, and two units in the air leave each other where they hang. That is decided first. Otherwise, when exactly one of the pair is the hero, the hero takes the hero push share of the overlap, a tunable, and the other unit the rest; two units that are not the hero take half each. A small share lets the hero hold a choke against a crowd walking into it while still pushing its own way through; at one half the hero is one more disc in the crowd. A push status moves the hero through the displacement above, which the share does not touch.
...
| A unit in the air | Moved by nothing: a push on it counts its ticks off without carrying it, and in a colliding pair the unit on the ground takes the whole overlap, the hero included whatever its push share; two in the air leave each other alone |
...
| Unit blocking | Positional push-out along the centre line, a capped number of passes the tunable sets, no speed change; all of it on the grounded unit when the other is in the air, decided first; the hero push share on the hero and the rest on the other when exactly one is the hero; half each otherwise |
