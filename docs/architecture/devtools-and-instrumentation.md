...
- **Hands over what the panel lists.** The tuning defaults, every definition number with its key and unit, the archetype ids, and what the last content reload came to, all read from the registry, so a new archetype or a new number appears in the panel without a code change.
