...
`DevApi` does three things and nothing else:

- **Submits commands.** A panel button becomes a `DebugCommand` variant — spawn a pack here, damage the hero, set an orb level, kill everything, pause, step one tick — or a `SetTuning` command for a slider. Each goes into the world's command buffer with the next tick's stamp, exactly as a click does.
...
```typescript
window.DevApi = { submit, view, rings, /* … */ }
```
...
| What it does | Submits commands, reads the world view, reads the instrumentation rings |
...
| Measurements | Tick time, render time, live counts, pool misses, event overwrites, frame rate |
| Who writes | The driver, `PlayScene`, the world, the pools, the event ring |
