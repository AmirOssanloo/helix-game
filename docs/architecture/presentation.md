...
| Damage numbers | A fixed set of `BitmapText` at the text band, sized for the busiest fight's hits inside one rise plus a margin, spawned where a hit landed, rising and fading by the tick count and the fraction; the oldest recycled when the set is full, and counted |
