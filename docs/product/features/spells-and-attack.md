...
The ten spells the hero can invoke, the attack, and the three damage types. Every spell has a targeting kind, a cast point, a cooldown, and a mana cost. The cooldown and the mana cost scale with the lowest orb level among the orbs in the spell's recipe; each number in its effects scales with the one orb its table names.
...
| --- | --- | --- | --- |
| QQQ | Hoarfrost | Unit | A status on one enemy: every hit it takes for the duration also stuns it briefly and deals bonus damage, at most once per short internal cooldown. Scales with Quartz |
...
| EEQ | Emberling | None, self | A summon: one player-owned unit next to the hero that attacks nearby enemies for its lifetime and cannot be ordered. Scales with Ember and Quartz |
...
Every spell has a cast point and a short backswing after it, and the numbers per spell are in its definition file. The hero stands still during a cast point; S or a new order cancels it at no cost. A move, a stop, or another cast cuts the backswing short.
...
| --- | --- | --- |
| Physical | Armour | Every attack: the hero's, Emberling's, and every enemy's |
...
| Attack-target on a unit that becomes untargetable, as a lifted unit does | The order drops to idle |
| Mana falls short of the cost during the cast point | The cast is cancelled at no cost |
| Siphon lands on an enemy with no mana | Nothing is burned and it takes no damage |
