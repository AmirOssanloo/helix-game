...
|---|---|---|
| Damage area | `target`; damage type physical, magical, or pure; `amount`, a table; `rate`, once or per second, per second being legal only in a zone's each-tick list and converted to per tick at load; `split`, whether the amount is divided evenly among the units hit | Zenith, Bolide, Clarion, the Hoarfrost hook; an enemy's arrow and slam |
...
Four functions, one file each under `src/domain/abilities/effects/`, each taking the world, the cast context, and its own fields, and nothing else. Three are the spells'; the fourth is an enemy's.
...
| `charge_to` | `distance`, a table; `speed`, in units a second; the status id it holds the caster with | Carries the caster itself toward the cast's target unit, or its anchor when it names none, for `distance` or up to the target's edge, bound radius to bound radius, whichever is shorter, as a push through the movement step, so a wall stops it; puts the status, `charge`, which raises the displaced flag, on the caster for the ticks the travel takes. Used by an enemy's `charge`, whose behaviour attacks from where it lands |
