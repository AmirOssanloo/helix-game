...
| Spawn projectile | `origin`, the context's anchor or the caster, a caster origin leaving from the caster toward the anchor; a speed, a radius, homing on the target or not, a maximum range, an on-hit effect list, a frame and tint | No spell. The auto-attack and the summon's attack fire one of their own; an enemy's root net fires one from the caster |
