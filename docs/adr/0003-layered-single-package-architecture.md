...
- **Simulation cost is bounded by a live cap, not the map.** Packs on a large map sit dormant as spawn data until the hero comes within an activation radius, a rule in the AI module that measures the hero against each record, since a record is not a unit and the hash does not hold it.
- **The hero is one unit with a stable id, and a form is a record it points at.** A form is run-scoped state: a definition (body, attributes, growth, ability list, kit key, atlas frame), health and mana, kit state such as an orb buffer, and an armory. The unit holds the active form's index and its shared state: position, facing, order, statuses, level, experience, and item slots. Systems read body and abilities through the active form each tick and never cache a definition across ticks. Cooldown clocks stay on the unit keyed by ability id, so a form's abilities keep independent clocks and a shared item's clock is shared, with no special case. A swap changes the index; every id in the world that pointed at the hero still does.
- **A slot key is a slot index, not a mechanic.** The input mapper turns Q, W, E, R, D, F into a command naming slots 1 to 6, and the active form's kit decides what a slot means. Invoke is one kit, resolved by key from the form definition like an effect or a behaviour; a plain hotbar kit maps a slot straight to an ability. The HUD draws the active kit's six slot descriptors, never a fixed Invoke layout.
