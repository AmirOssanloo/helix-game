...
/** Every command a silence leaves alone. */
...
describe("a silence on the hero", () => {
  it.each([
    ["slot", { kind: "slot", slot: Q } as const],
    [
      "cast",
      {
        kind: "cast",
  ...
  ])("refuses %s: a silence blocks the ability keys", (_name, command) => {
    const { world, reader } = arrange();
    apply(world, "silence", LONG_TICKS);
    world.tick();
  ...
  it.each(ALLOWED)(
    "accepts %s: movement and attacks continue",
    (_name, command) => {
      const { world, reader } = arrange();
      apply(world, "silence", LONG_TICKS);
      world.tick();
  ...
  it("leaves a move running: the hero keeps walking", () => {
