...
/** Short enough that a spec ticks past the end of the status. */
const SHORT_TICKS = 3;
...
describe("a root on the hero", () => {
  it.each([
    ["move", { kind: "move", destination: { x: 500, y: 0 } } as const],
    [
      "attack_move",
      { kind: "attack_move", destination: { x: 500, y: 0 } } as const,
    ],
  ])("refuses %s: a root blocks movement", (_name, command) => {
  ...
  it.each(ALLOWED)(
    "accepts %s: attacks and spells continue",
    (_name, command) => {
      const { world, reader } = arrange();
      apply(world, "root", LONG_TICKS);
      world.tick();
  ...
  it("clears a move under way, and the hero does not resume it when the root ends", () => {
