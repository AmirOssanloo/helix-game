...
describe("the projection", () => {
  it("draws at the one scale the game has: a diamond 40 by 20, k = 0.625", () => {
    expect(DIAMOND_WIDTH).toBe(40);
    expect(VIEW_SCALE).toBe(0.625);
  });
  ...
  it("draws the four corners of a cell as a diamond that many whole pixels across and half that down", () => {
    ...
    expect(right.x - left.x).toBe(DIAMOND_WIDTH);
    expect(bottom.y - top.y).toBe(DIAMOND_WIDTH / 2);
  ...
  it("stands the top of a circle a radius times k over root two above its centre", () => {
    ...
    expect(projection.riseOf(radius)).toBeCloseTo(radius * k * Math.SQRT1_2, 9);
  });
});
